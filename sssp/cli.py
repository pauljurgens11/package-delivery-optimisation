...
def _build_graph_from_file(path: str, fmt: Optional[str]) -> Tuple[Graph, int, int, Optional[int]]:
    """Build a :class:`Graph` from an edges file.

    Returns:
        (G, n, m, file_source) where ``file_source`` is the source vertex
        encoded in the file, when available (currently only for the ``txt``
        edge-list format produced by ``generator/graph_generator.py``).
    """
    p = Path(path)
    if not p.exists():
    ...
    # For generator-produced .txt graphs, prefer the header's source when the
    # user does not override it explicitly via CLI arguments.
    if (fmt == "txt") or (fmt is None and p.suffix.lower() == ".txt"):
        try:
            from generator.graph_generator import load_edge_list_txt  # type: ignore[import]
        except Exception as exc:  # pragma: no cover - import environment
            raise InputError(
                "failed to import generator.graph_generator.load_edge_list_txt "
                "needed for txt edge-list files"
            ) from exc

        gen = load_edge_list_txt(str(p))
        file_source = int(gen.source)
        from .graph import Graph as SSSPXGraph

        G = SSSPXGraph.from_edges(gen.n, gen.edges)
    else:
        G = read_graph(path, fmt)

    m = sum(len(lst) for lst in G.adj)
    return G, G.n, m, file_source
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    p.add_argument(
        "--format",
        choices=["csv", "jsonl", "mtx", "graphml", "txt"],
        default=None,
        help="Edge file format (auto-detected from extension). Use 'txt' for "
        "graphs generated by generator/graph_generator.py.",
    )

    p.add_argument(
        "--algo",
        choices=["bmssp", "dijkstra", "bellman-ford"],
        default="bmssp",
        help="SSSP algorithm to run: BMSSP-style solver, Dijkstra baseline, or Bellman–Ford baseline.",
    )
    ...
    src_group.add_argument(
        ...
        type=int,
        default=None,
        help="Source vertex id. If omitted for txt edge-list files, the value from the file header is used.",
    )
    src_group.add_argument(
        "--sources",
        type=str,
    ...
    p.add_argument(
        ...
        type=str,
        default=None,
        help="Write the main JSON result (distances, optional path) to this file "
        "instead of stdout.",
    )
    ...
    try:
        # Build graph
        file_source: Optional[int]
        if args.random:
            G = _build_random_graph(args.n, args.m, args.seed)
            n, m = args.n, args.m
            file_source = None
        else:
            G, n, m, file_source = _build_graph_from_file(args.edges, args.format)
        ...
        from .baselines import DijkstraSolver, BellmanFordSolver

        if args.algo == "bmssp":
            SolverCls = SSSPSolver
        elif args.algo == "dijkstra":
            SolverCls = DijkstraSolver
        else:
            SolverCls = BellmanFordSolver
        ...
        if args.metrics_out:
            ...
            if args.profile:
                with ProfileSession(dump_path=args.profile_out) as prof:
                    solver = SolverCls(G, sources[0], config=cfg, logger=logger, sources=sources)
                    res = solver.solve()
